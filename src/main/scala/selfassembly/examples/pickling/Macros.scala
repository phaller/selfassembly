...
trait PicklerMacros extends Query[Unit] {
  override def mkTrees[C <: Context with Singleton](c: C) = new Trees(c)
  ...
  class Trees[C <: Context with Singleton](override val c: C) extends super.Trees(c) {
    ...
    override def putField(getterLogic: Tree, innerMostPickleLogic: c.Tree, nameOfSubPicklee: c.TermName,
      ...
      wrap {
        val elided =
          if (fieldTpe.typeSymbol.isEffectivelyFinal) q"b.hintStaticallyElidedType()"
          else q"if ($nameOfSubPicklee == null || $nameOfSubPicklee.getClass == classOf[$fieldTpe]) b.hintDynamicallyElidedType() else {}"
        q"""
    ...
    override def combine(left: c.Expr[Unit], right: c.Expr[Unit]): c.Expr[Unit] =
      c.Expr(q"$left ; $right")

    override def delimit(tpe: c.Type): (c.Expr[Unit], c.Expr[Unit], c.Expr[Unit]) = {
...
trait UnpicklerMacros extends AcyclicQuery[Any] {
  ...
  class Trees[C <: Context with Singleton](override val c: C) extends super.Trees(c) {
    ...
    override def combine(left: c.Expr[Any], right: c.Expr[Any]): c.Expr[Any] = left

    override def delimit(tpe: c.Type): (c.Expr[Any], c.Expr[Any], c.Expr[Any]) =
...
trait PickleMacros extends Macro {
  ...
  def pickleInto[T: c.WeakTypeTag](builder: c.Tree): c.Tree = {
    ...
    q"""
      ...
      val pickler = implicitly[SPickler[$tpe]]
