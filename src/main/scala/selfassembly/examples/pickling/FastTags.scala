...
trait FastTypeTagMacros extends Macro {
  def impl[T: c.WeakTypeTag]: c.Tree = {
    ...
    q"""
      new FastTypeTag[$T] {
        def mirror = selfassembly.examples.pickling.internal.`package`.currentMirror
  ...
  def apply(key: c.Tree): c.Tree = {
    import c.universe._
    q"""selfassembly.examples.pickling.FastTypeTag(selfassembly.examples.pickling.internal.`package`.currentMirror, $key)"""
  }
}
